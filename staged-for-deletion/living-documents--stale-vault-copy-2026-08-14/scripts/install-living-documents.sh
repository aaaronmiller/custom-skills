...
if [ "${1-}" = "--help" ]; then
  printf '%s\n' "Usage: install-living-documents.sh [--enable-renderer] [--install-hooks] [--install-rules] [--sync-skills]"
  exit 0
fi
...
if [ "$INSTALL_RULES" -eq 1 ]; then
  python3 "$ROOT/scripts/install-living-documents-rules.py" --apply
fi
