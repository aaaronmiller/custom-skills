...
It reconstructs the intent implied by a downstream artifact, compares that reconstruction to the upstream source of truth, and classifies the divergence. Divergence is the audit signal, not a verdict: round-trip agreement bounds drift but does not prove correctness, and an in-session reconstruction is not a blind audit. The method and its grounding (instruction backtranslation, round-trip correctness) are in `references/method.md`.
