...
description: '**DEPRECATED — Use spec-audit-skill-v3 instead.** (problems + our solutions),
  generate self-contained questions that describe ONLY the problem domain (no implementation
  details revealed), submit questions ALONE to an S-tier model, compare the model''s
  first-principles answers against the spec — the divergence IS the audit signal.


  Use when: user says "spec audit", "backtranslation audit", "compare our design against
  first principles", "blind spec review", "gauge our design against first principles",
  "test if our implementation is optimal", "external perspective on our design", "ideal
  spec from questions only", "path 2 spec audit", "backtranslate our design". Also
  triggers on "divergence analysis", "first-principles audit", "solution vs intuition
  comparison".


  v2 improvement over v1: Questions NEVER reveal implementation details. S-tier model
  answers from first principles only. Divergence between answers and spec IS the audit
  finding.'
tags:
- planning
grade: C
