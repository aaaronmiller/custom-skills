...
# Antigravity's installed-app OAuth client configuration is intentionally not
# embedded in this distributable skill. Supply it from the local environment.
...
def find_all_antigravity_credentials() -> list[dict]:
    ...
    # Priority 0: CLIProxyAPI Antigravity creds (~/.cli-proxy-api/antigravity-*.json)
    # Shape: {access_token, refresh_token, project_id, email, ...}
    import glob as _glob
    for path in sorted(_glob.glob(str(Path.home() / ".cli-proxy-api" / "antigravity-*.json"))):
        try:
            ...
            all_creds.append({
                ...
                "projectId": data.get("project_id") or data.get("projectId"),
            })
    ...
    if all_creds:
        ...
        print(f"Found {len(uniq)} Antigravity account(s) via CLIProxyAPI", file=sys.stderr)
        return uniq
...
def refresh_access_token(refresh_token: str) -> str:
    ...
    if not ANTIGRAVITY_CLIENT_ID or not ANTIGRAVITY_CLIENT_SECRET:
        raise RuntimeError(
            "Antigravity backend requires ANTIGRAVITY_OAUTH_CLIENT_ID and "
            "ANTIGRAVITY_OAUTH_CLIENT_SECRET in the local environment"
        )
