...
version: 3.1
description: |
  True backtranslation-driven spec audit, aligned with the LLM SFT training data
  generation technique.

  The process: write spec-as-designed.md (problem→solution pairs, clause-structured).
  Backtranslate each clause into a question whose CORRECT ANSWER IS that clause — the
  S-tier model answers cold, with no spec. Its answers, read in sequence, should
  reconstruct the spec. Divergence between the model's reconstruction and your actual
  spec IS the audit signal. Council deliberates on divergences. ideal-spec.md +
  task-list.md are the final output.

  v3.1 vs v3: v3 still contained residual v2 framing ("asks for the ideal solution"),
  lacked contamination guards on iteration, was missing question-level expected-obviousness
  ratings, had no built-vs-designed debt classification, and over-elevated Path 1.
  v3.1 corrects all of these. See CHANGELOG below.

  Also includes: constraint tagging, confidence scoring, coverage check, bounded
  iteration with contamination guard, built-vs-designed debt classification, question
  expected-obviousness ratings, divergence etiology, and optional Path 1 support.
  Resources for specific sub-tasks are listed below and loaded only when needed
  (progressive disclosure).

  Use when: "spec audit", "backtranslation audit", "blind spec review",
  "ideal spec from questions", "compare design against first principles",
  "divergence analysis", "test if our implementation is optimal",
  "external perspective on our design", "implementation debt check",
  "design debt check".
---

# Spec Audit Skill v3.1 — True Backtranslation Audit
...
| 7 | Added stub content to all 6 resource files | Skill is now self-contained; resources can be expanded later |
...
└── Deliberative refinement on AMBIGUOUS and DIVERGENT-BETTER items
      └── See resource: resources/council-formations.md
...
> See resource: `resources/built-vs-designed-guide.md` for detailed worked examples.
...
> See `resources/council-formations.md` for available council formations and
> selection criteria.
...
> See resource: `resources/dual-path-merge-guide.md` for detailed merge procedures.
...
### `resources/built-vs-designed-guide.md`
**When to load:** Phase 1b and Phase 4, when running the built-vs-designed
comparison (only if spec-as-built.md exists).
**What it covers:** How to generate parallel questions from spec-as-built.md,
how to avoid cross-contamination between designed and built question sets,
worked examples of implementation-debt vs design-debt classification,
how debt classification drives different task-list priorities.
**Location:** `[skill-root]/resources/built-vs-designed-guide.md`
